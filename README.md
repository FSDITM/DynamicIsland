...
```
установившийся режим: 60.0 fps, CPU 7.1% одного ядра, 1.18 ms CPU на кадр
первый кадр 70 ms (компиляция шейдеров, разово)
в простое: 0 кадров, поток спит
```
...
- Часы и заряд батареи в компактном виде
- При наведении раскрывается в плеер: обложка, название, исполнитель,
  перемотка и пауза — через штатный Windows-API
